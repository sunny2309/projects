...
def main():
    ...
    if tgood:
        for u in tgood:
            ...
            s, g, u = score(h, u, reverse)
            #print(s,g,u)
            #print(type(s), type(g), type(u))
            y_preds_ham = np.hstack((s*[1], g*[0], u*[0.5]))
            y_true_ham = np.array(len(y_preds_ham)*[0])
            spams += s
            hams += g
            unsures += u
            fp += s 
    if tspam:
        for u in tspam:
            ...
            #print(s,g,u)
            #print(type(s), type(g), type(u))
            y_preds_spam = np.hstack((s*[1], g*[0], u*[0.5]))
            y_true_spam = np.array(len(y_preds_spam)*[1])
            spams += s
            hams += g
            unsures += u
            fn += g 
    
    y_true = np.hstack((y_true_ham, y_true_spam))
    y_preds = np.hstack((y_preds_ham, y_preds_spam))
    ...
    with open("results.txt", "a") as f:
        f.write("=" * 78)
        f.write("\n")
        if 'enron' in os.path.basename(train_path):
            f.write("dataset: %s\n" % os.path.basename(train_path))
        elif 'trec07' in os.path.basename(train_path):
            f.write("dataset: %s\n" % '_'.join(os.path.basename(train_path).split('_')[1:]))
        else:
            f.write("dataset: %s\n" % os.path.basename(train_path).split('_')[1])
        f.write("-" * 78)
        f.write("\n")
        ...
        f.write("F1-Score : %.3f"%f1_score(y_true, y_preds))
        f.write("\n")
        f.write("Classification Report : \n")
        f.write(str(classification_report(y_true, y_preds)))
        f.write("\n")
