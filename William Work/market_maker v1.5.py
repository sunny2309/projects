...
class MarketMaker:
    ...
    def grab_current_inventory(self):
        ...
        print("Current Net Liquidation Value        : $ {}".format(self.current_net_liquidation_value))
        print("Current Position Value               : $ {}".format(self.current_position_value))
        print("Current Position Quantity            : {}".format(self.current_position_qty))
        print("Current Inventory percentage         : {}".format(self.current_inventory_pct))
    ...
    def reserve_price(self):
        ...
        print("Current Reserve Price                : $ {}".format(self.current_reserve_price))
    ...
    def OnData(self):
        ...
        if self.current_inventory_pct > 0:
            print('Inventory > 0', self.current_net_liquidation_value, self.max_short_position_balance, self.max_long_position_balance, self.current_position_value)
        ...
        sell_order_qty = ask_size_dollar_amount / ask_price
        buy_order_qty = bid_size_dollar_amount / bid_price
        ...
        print("Sell Quantity                        : {}".format(sell_order_qty))
        print("Bid Price                            : $ {}".format(bid_price))
        print("Buy Quantity                         : {}".format(buy_order_qty))
        print("ask_size_dollar_amount               : {}".format(ask_size_dollar_amount))
        print("bid_size_dollar_amount               : {}".format(bid_size_dollar_amount))

        print(self.current_inventory_pct, self.current_net_liquidation_value, self.max_long_position_balance, self.max_short_position_balance)

        print("==================== END ==============================\n")
        
        sell_order_status = self.kraken.trade(self.instrument[0].replace("/", ""), 'sell', 'limit', (.99 * sell_order_qty), price="{:.2f}".format(ask_price), leverage=2) # Sell order
        if sell_order_status:
            buy_order_status = self.kraken.trade(self.instrument[0].replace("/", ""), 'buy', 'limit', (.99 * buy_order_qty), price="{:.2f}".format(bid_price), leverage=2) # Buy order
